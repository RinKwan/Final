...
class DirectionalLight extends Light {
    ...
    update() {
        ...
        if (App.which_ligth == 1) {
            let osize = 5;
            let lightProjection = mat4.ortho(mat4.create(), -osize, osize, -osize, osize, -osize, osize)
            let ligthView = mat4.lookAt(mat4.create(), direction, [0.00001, 0.00003, 0.00002], [0, 1, 0])
            let lightSpaceMatrix = mat4.multiply(mat4.create(), lightProjection, ligthView)

            for (var i = 0; i < App.all_shaders.length; i++) {
                if (App.all_shaders[i] == this.target_shader) {
                    //console.log('A' + i)
                }
            }

            let lightSpaceMatrixLocation = this.target_shader.getUniformLocation('lightSpaceMatrix');
            if (lightSpaceMatrixLocation != null)
                this.gl.uniformMatrix4fv(lightSpaceMatrixLocation, false, lightSpaceMatrix)
        }
...
class PointLight extends Light {
    ...
    update() {
        ...
        //---
        if (App.which_ligth == 0) {
            let osize = 5;
            let lightProjection = mat4.ortho(mat4.create(), -osize, osize, -osize, osize, -osize, osize)
            let ligthView = mat4.lookAt(mat4.create(), position, [0.00001, 0.00003, 0.00002], [0, 1, 0])
            let lightSpaceMatrix = mat4.multiply(mat4.create(), lightProjection, ligthView)
